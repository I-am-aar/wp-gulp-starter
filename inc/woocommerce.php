...
function _themename_woocommerce_scripts() {
	wp_enqueue_style( '_themename-woocommerce-style', get_template_directory_uri() . '/woocommerce.css' );
 ...
 */
function _themename_woocommerce_products_per_page() {
	return 12;
}
add_filter( 'loop_shop_per_page', '_themename_woocommerce_products_per_page' );
 ...
 */
function _themename_woocommerce_thumbnail_columns() {
	return 4;
}
add_filter( 'woocommerce_product_thumbnails_columns', '_themename_woocommerce_thumbnail_columns' );
 ...
 */
function _themename_woocommerce_loop_columns() {
	return 3;
}
add_filter( 'loop_shop_columns', '_themename_woocommerce_loop_columns' );
...
if ( ! function_exists( '_themename_woocommerce_product_columns_wrapper' ) ) {
	/**
	 * Product columns wrapper.
	 *
	 * @return  void
	 */
	function _themename_woocommerce_product_columns_wrapper() {
		$columns = _themename_woocommerce_loop_columns();
		echo '<div class="columns-' . absint( $columns ) . '">';
	}
}
add_action( 'woocommerce_before_shop_loop', '_themename_woocommerce_product_columns_wrapper', 40 );

if ( ! function_exists( '_themename_woocommerce_product_columns_wrapper_close' ) ) {
	/**
	 * Product columns wrapper close.
	 *
	 * @return  void
	 */
	function _themename_woocommerce_product_columns_wrapper_close() {
		echo '</div>';
	}
}
add_action( 'woocommerce_after_shop_loop', '_themename_woocommerce_product_columns_wrapper_close', 40 );
	...
	function _themename_woocommerce_cart_link() {
		...
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', '_themename' ); ?>">
			...
			$item_count_text = sprintf(
				...
				_n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), '_themename' ),
